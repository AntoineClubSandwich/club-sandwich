-- Bilan RSE des tourneurs : "repas estimés" ne reflète jamais rien de
-- réel - complete_guided_maraude crée le rapport opérationnel avec
-- estimated_meals à null en dur (20260825000000), et rien ne le
-- calcule ni ne le remplit automatiquement ensuite. La seule vraie
-- donnée de repas est distributed_meals, issue de la distribution
-- réellement saisie par l'équipe (save_maraude_distribution_v4).
-- Retire la colonne "repas estimés" de l'export et ne garde que le
-- chiffre réel.

drop function public.get_maraude_export_rows(date, date, uuid);

create function public.get_maraude_export_rows(
